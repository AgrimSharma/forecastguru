...
def live_forecast(request):
    ...
    try:
        user = request.user
        profile = SocialAccount.objects.get(user=user)
        forecast_live = ForeCast.objects.filter(approved__name="yes", private__name='no', status__name='In-Progress').order_by("expire")
...
def forecast_result(request):
    ...
    try:
        ...
        forecast_live = ForeCast.objects.filter(approved__name="yes", private__name='no',  status__name='Result Declared').order_by(
            "-expire")
        for f in forecast_live:
            date = current.date()
            bet_start = (f.expire + datetime.timedelta(hours=5, minutes=30)).date()
...
def my_forecast(request):
    ...
    forecast_live = Betting.objects.filter(forecast__approved__name="yes", forecast__private__name='no',  forecast__status__name='In-Progress',users=account, forecast__private__name='no').order_by("forecast__expire")
    forecast_result = Betting.objects.filter(forecast__approved__name="yes", forecast__private__name='no',  forecast__status__name='Result Declared',users=account, forecast__private__name='no').order_by("forecast__expire")
...
def search_result(request):
    if request.method == "POST":
        ...
        if query == "":
            ...
            forecast_live = ForeCast.objects.filter(heading__icontains=query, private__name='no',  approved__name="yes", status__name='In-Progress').order_by(
                "-expire")
...
def live_forecast_data_bet(forecast_live, account):
    ...
    for f in forecast_live:
        ...
        try:
            ...
            bet_for_user = Betting.objects.filter(forecast=forecast, users=account).aggregate(bet_for=Sum('bet_for'))['bet_for']
            bet_against = Betting.objects.filter(forecast=forecast).aggregate(bet_against=Sum('bet_against'))['bet_against']
            bet_against_user = Betting.objects.filter(forecast=forecast, users=account).aggregate(bet_against=Sum('bet_against'))['bet_against']
            totl = bet_against + bet_for
            percent_for = (bet_for / totl) * 100
            percent_against = (100 - percent_for)
...
def forecast_live_view(category, profile):
    ...
    for f in forecast_live:
        ...
        try:
            total_wagered = betting_against + betting_for
            bet_for = Betting.objects.filter(forecast=forecast).aggregate(bet_for=Sum('bet_for'))['bet_for']
            bet_for_user = Betting.objects.filter(forecast=forecast, users=profile).aggregate(bet_for=Sum('bet_for'))['bet_for']
            bet_against = Betting.objects.filter(forecast=forecast).aggregate(bet_against=Sum('bet_against'))['bet_against']
            bet_against_user = Betting.objects.filter(forecast=forecast, users=profile).aggregate(bet_against=Sum('bet_against'))['bet_against']
            totl = bet_against + bet_for
            percent_for = (bet_for / totl) * 100
            percent_against = (100 - percent_for)
...
def forecast_result_view(category, profile):
    data = []

    forecast_live = ForeCast.objects.filter(approved__name="yes", private__name='no',  category=category, status__name='Closed').order_by(
        "expire")
    for f in forecast_live:
        forecast = f
        date = current.date()
...
def forecast_result_view_sub(category, profile):
    data = []

    forecast_live = ForeCast.objects.filter(approved__name="yes", private__name='no',  sub=category, status__name='Closed').order_by(
        "expire")
    for f in forecast_live:
        forecast = f
        date = current.date()
...
def forecast_result_view_bt(category_id):
    data = []

    forecast_live = ForeCast.objects.filter(approved__name="yes", private__name='no',  category=category_id, status__name='Closed').order_by(
        "expire")
    for f in forecast_live:
        forecast = f
        date = current.date()
...
def forecast_result_view_bt_sub(category_id):
    data = []

    forecast_live = ForeCast.objects.filter(approved__name="yes", private__name='no',  sub_category=category_id, status__name='Closed').order_by(
        "expire")
    for f in forecast_live:
        forecast = f
        date = current.date()
